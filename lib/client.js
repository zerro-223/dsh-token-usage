window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TrendChart(props) {
			...
			// Token curves share the left axis; the cost curve uses a right axis.
			const maxOfLeft = (arr) => arr.reduce((m, d) => Math.max(m, d.inputTokens, d.outputTokens, d.cacheReadTokens), 0) || 1;
			const maxOfRight = (arr) => arr.reduce((m, d) => Math.max(m, d.costUsd || 0), 0) || 1;
			const dataMaxL = maxOfLeft(data);
			const dataMaxR = maxOfRight(data);
			...
			const monotoneSlopes = (arr, key) => {
				...
				for (let i = 0; i < len - 1; i += 1) slope[i] = dy[i];
				slope[len - 1] = slope[len - 2];
				for (let i = 1; i < len - 1; i += 1) {
					if (slope[i - 1] * slope[i] <= 0) slope[i] = 0;
					else slope[i] = (slope[i - 1] + slope[i]) / 2;
				}
				return slope;
			};
			...
			if (morph && morph.fromData && morph.progress < 1) {
				...
				const maxFromL = maxOfLeft(morph.fromData);
				const maxToL = dataMaxL;
				const maxFromR = maxOfRight(morph.fromData);
				...
				series = SEG_META.map((s) => {
					...
					for (let i = 0; i < MORPH_SAMPLES; i += 1) {
						const v = fromVals[i] + (toVals[i] - fromVals[i]) * t;
						const y = s.axis === "right" ? yOfR(v, maxR) : yOfL(v, maxL);
						pts.push({ x: PAD.l + xNorm(i) * plotW, y });
					}
					return { dash: s.dash === true, axis: s.axis, pts };
				});
			} else {
				series = SEG_META.map((s) => ({
					dash: s.dash === true,
					axis: s.axis,
					pts: data.map((d, i) => ({ x: xOf(i), y: s.axis === "right" ? yOfR(d[s.key], dataMaxR) : yOfL(d[s.key], dataMaxL) })),
				}));
			}
			...
			const axisMetaUsd = (max) => ({
				text: (v) => (max >= 1 ? v.toFixed(2) : v.toFixed(4)),
			});
								...
								children: [
									...
									SEG_META.map((s) => h("div", {
										...
										children: [
											...
											h("span", { className: "ts-tipVal", children: s.key === "costUsd" ? fmtUsd(data[hover][s.key]) : fmtNum(data[hover][s.key]) }),
										],
									})),
									h("div", {
										className: "ts-tipRow",
							...
							h("div", {
								...
								children: [
									...
									exportOpen ? h("div", {
										...
										children: [
											h("button", { type: "button", className: "ts-exportMenuItem", onClick: exportJson, children: t("export.json") }),
								...
								children: [
									...
									days === "custom" ? h("div", {
										...
										children: [
											...
											h("span", { className: "ts-customRangeSep", children: "~" }),
											h("input", {
												type: "date",
												className: "ts-customDate",
												value: customEnd,
								...
								children: [
									error && !data ? h(ErrorState, { t, message: error, onRetry: refreshAll })
										...
										: [
											...
											h(ContributionHeatmap, { data: heatmap.data ? heatmap.data.series : [], t, delay: 280 }),
											h(ModelSummary, {
												models: data.byModel || [],
												t,
												selectedModel: model,
											...
											data.recent && data.recent.length > 0 ? h(RecentList, { rows: data.recent.slice(0, 8), t, delay: 340, data }) : null,
										],
									],
								},
							),
