window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function usePrices() {
			...
			return { prices, auto, autoUpdatedAt, autoBusy, load, save, refreshAuto };
		}
		...
		function PricePanel(props) {
			const { models, prices, auto, autoUpdatedAt, autoBusy, onRefreshAuto, onClose, onSave, t } = props;
			const [draft, setDraft] = react.useState(() => {
				const base = {};
				for (const m of models) base[m] = { input: "", output: "", cacheRead: "" };
				for (const [m, p] of Object.entries(prices || {})) {
					base[m] = base[m] || { input: "", output: "", cacheRead: "" };
					if (p.input !== undefined) base[m].input = String(p.input);
					if (p.output !== undefined) base[m].output = String(p.output);
					if (p.cacheRead !== undefined) base[m].cacheRead = String(p.cacheRead);
				}
				return base;
			});
			const setField = (model, field, value) => {
				setDraft((prev) => {
					const row = { ...(prev[model] || { input: "", output: "", cacheRead: "" }) };
					row[field] = value;
			...
			const hasAny = models.some((m) => {
				const row = draft[m];
				return row && (row.input !== "" || row.output !== "" || row.cacheRead !== "");
			});
			return h("div", {
				...
				children: [
					...
					h("div", {
						...
						children: models.length === 0
							...
							: models.map((m) => {
								const autoRow = auto ? auto[m] : null;
								const hasManual = draft[m] && (draft[m].input !== "" || draft[m].output !== "" || draft[m].cacheRead !== "");
								return h("div", {
									...
									children: [
										...
										h("span", {
											...
											children: hasManual ? t("price.manualTag") : autoRow ? t("price.autoTag") : "",
										}),
										h("button", {
											type: "button",
											className: "ts-priceClear",
											title: t("price.clear"),
											onClick: () => setField(m, "input", "") || setField(m, "output", "") || setField(m, "cacheRead", ""),
		...
		function TokenStatsPanel(props) {
			...
			const priceModels = data
				? [...new Set([...data.filterOptions.models, ...Object.keys(priceApi.prices || {})])].sort()
				: [];
		...
		const zh = {
			...
			"price.autoSource": "价格源：modelradar.cn",
		...
		const en = {
			...
			"price.autoSource": "Source: modelradar.cn",
