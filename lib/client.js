window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SEG_META = [
			{ key: "inputTokens", labelKey: "legend.input", color: "var(--dsw-static-deepseek-500)" },
			{ key: "outputTokens", labelKey: "legend.output", color: "var(--dsw-static-amber-500)" },
			{ key: "cacheReadTokens", labelKey: "legend.cacheHit", color: "var(--dsw-static-green-500)", dash: true },
		];
		//#endregion
		...
		function TrendChart(props) {
			const { data, granularity, t, delay, animateKey } = props;
			...
			const PLOT_H = 236;
			const PAD = { l: 46, r: 24, t: 16, b: 28 };
			const n = data.length;
			const plotW = Math.max(width - PAD.l - PAD.r, 1);
			const baseY = PLOT_H - PAD.b;
			// Single shared linear y-axis for input, output and cache hits, so the
			// three curves are directly comparable (no misleading dual scales).
			const max = data.reduce((m, d) => Math.max(m, d.inputTokens, d.outputTokens, d.cacheReadTokens), 0) || 1;
			const xOf = (i) => (n <= 1 ? PAD.l + plotW / 2 : PAD.l + (i / (n - 1)) * plotW);
			const yOf = (v) => PAD.t + (1 - v / max) * (baseY - PAD.t);
			...
			const series = SEG_META.map((s) => ({
				dash: s.dash === true,
				pts: data.map((d, i) => ({ x: xOf(i), y: yOf(d[s.key]) })),
			}));

			// Single y-axis ticks with one shared unit (M token / K token / token).
			const axisMeta = (max) => ({
			...
			const meta = axisMeta(max);
			const midY = PAD.t + (baseY - PAD.t) / 2;
			const ticks = [
				{ y: PAD.t, text: meta.text(max) },
				{ y: midY, text: meta.text(max / 2) },
				{ y: baseY, text: "0" },
			];
		...
		function TokenStatsPanel(props) {
			...
			const heatmap = useOverview("364", provider, model);
			const refreshAll = () => { refresh(); heatmap.refresh(); };
			const chartAnimateKey = days + "|" + provider + "|" + model;
								...
								children: [
									error && !data ? h(ErrorState, { t, message: error, onRetry: refreshAll })
										...
										: [
											...
											h(TrendChart, { data: data.series, granularity: data.granularity, t, delay: 220, animateKey: chartAnimateKey }),
		...
		const zh = {
			...
			"chart.trendHint": "三条曲线：输入 / 输出 / 缓存命中",
			"legend.input": "输入",
		...
		const en = {
			...
			"chart.trendHint": "three lines: input / output / cache hit",
			"legend.input": "Input",
