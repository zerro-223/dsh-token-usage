window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Chinese unit conversion (万/亿) for the total-tokens hint, e.g. 111111 -> "≈11万"
		...
		function PricePanel(props) {
			const { models, prices, onClose, onSave, t } = props;
		...
		function TokenStatsPanel(props) {
			...
			const statCards = totals ? [
				...
				{ label: t("stat.cost"), value: data ? data.cost.usd : null, sub: data && data.cost.usd === null ? t("cost.unpriced") : data && data.cost.unknownModels.length > 0 ? t("cost.partial") : null, accent: "Red", delay: 340, format: "usd" },
			] : null;
