...
/** Manual price lookup: own property only, so model ids like "constructor"
 * or "toString" cannot hit Object.prototype. */
function manualPrice(manualPrices, model) {
  return Object.prototype.hasOwnProperty.call(manualPrices, model) ? manualPrices[model] : undefined;
}
...
export function calcCost(records, manualPrices, autoIndex) {
  ...
  for (const r of records) {
    const p = manualPrice(manualPrices, r.model) ?? (autoIndex ? autoIndex.get(normModelKey(r.model)) : undefined);
    if (p === undefined) {
      unknown.add(r.model);
      continue;
    }
