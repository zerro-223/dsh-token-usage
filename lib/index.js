...
* @param prices - model price table (USD per 1M tokens).
*/
function buildOverview(records, range, provider, model, prices) {
  const now = Date.now();
  ...
  const cost = calcCost(filtered, prices);
...
export function apply(ctx) {
  ...
  const handler = async (req, res) => {
    ...
    if (url.pathname === '/token-stats/api/overview') {
      ...
      const payload = buildOverview(records, range, provider, model, prices);
