...
function buildAutoIndex(entries) {
  const index = new Map();
...
*/
function calcCost(records, manualPrices, autoIndex) {
  let usd = 0;
  let anyConfigured = false;
  const unknown = new Set();
  for (const r of records) {
    const p = manualPrices[r.model] ?? autoIndex.get(normModelKey(r.model));
    if (p === undefined) {
      unknown.add(r.model);
      continue;
    }
...
export function apply(ctx) {
  ...
  async function refreshAutoPrices() {
    try {
      ...
      for (const m of list) {
        ...
        const row = {};
        if (Number.isFinite(Number(m.inputPricePer1M))) row.input = Number(m.inputPricePer1M);
        if (Number.isFinite(Number(m.outputPricePer1M))) row.output = Number(m.outputPricePer1M);
        if (Number.isFinite(Number(m.cacheReadPricePer1M))) row.cacheRead = Number(m.cacheReadPricePer1M);
        if (typeof m.name === 'string' && m.name !== '') row._name = m.name;
        if (Object.keys(row).length > 1) entries[m.id] = row;
      }
      autoEntries = entries;
  ...
  // Background refresh: immediately once, then daily.
  ctx.effect(() => {
    const immediate = setTimeout(() => {
      refreshAutoPrices().then((r) => {
  ...
  const handler = async (req, res) => {
    ...
    if (url.pathname === '/token-stats/api/prices' && req.method === 'GET') {
      sendJson(res, 200, { prices, auto: autoEntries, autoUpdatedAt });
      return;
    }
