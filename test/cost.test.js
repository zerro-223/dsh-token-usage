...
test('calcCost bills cacheWriteTokens at its own price', () => {
  const r = rec({ cacheWriteTokens: 1_000_000 });
  const prices = { 'deepseek-chat': { input: 1, output: 2, cacheRead: 0.5, cacheWrite: 0.8 } };
  const { usd, unknownModels } = calcCost([r], prices);
  assert.equal(unknownModels.length, 0);
  // 1M cache-write tokens at $0.8/M = $0.8
  assert.equal(usd, 0.8);
});

test('calcCost treats a missing cacheWrite price as 0', () => {
  const r = rec({ cacheWriteTokens: 1_000_000 });
  const prices = { 'deepseek-chat': { input: 1, output: 2, cacheRead: 0.5 } };
  assert.equal(calcCost([r], prices).usd, 0);
});
...
test('calcCost bills all disjoint buckets together', () => {
  ...
  const prices = { 'deepseek-chat': { input: 1, output: 2, cacheRead: 0.5, cacheWrite: 0.75 } };
  // 1*1 + 2*2 + 3*0.5 + 4*0.75 = 1 + 4 + 1.5 + 3 = 9.5
  assert.equal(calcCost([r], prices).usd, 9.5);
});
...
test('calcCost treats missing price fields as 0', () => {
  ...
  // input 1*1 + output 0 + cacheWrite 0 = 1
  assert.equal(calcCost([r], prices).usd, 1);
});

test('sanitizePrices keeps cacheWrite keys', () => {
  const raw = { m: { input: 1, output: 2, cacheRead: 0.5, cacheWrite: 1.5 } };
  assert.deepEqual(sanitizePrices(raw), { m: { input: 1, output: 2, cacheRead: 0.5, cacheWrite: 1.5 } });
});
...
test('calcCost handles very large token counts', () => {
  ...
  const prices = { 'deepseek-chat': { input: 100, cacheWrite: 100 } };
  // 1e12 / 1e6 * 100 = 1e8
...
test('buildAutoIndex indexes id, provider-prefixed basename and name', () => {
  const entries = {
    'gpt-5.6-sol': { input: 5, output: 6.25, _name: 'GPT-5.6-sol', _currency: 'USD' },
  };
  const idx = buildAutoIndex(entries);
  assert.equal(idx.get('gpt56sol'), entries['gpt-5.6-sol']);       // exact id
  assert.equal(idx.get('gpt56sol'), entries['gpt-5.6-sol']);       // basename of "openai/gpt-5.6-sol"
  assert.equal(idx.get('gpt56sol'), entries['gpt-5.6-sol']);       // display name
  assert.equal(idx.get('openai/gpt56sol'), undefined);
});

test('buildAutoIndex rejects non-object input', () => {
  assert.equal(buildAutoIndex(null).size, 0);
  assert.equal(buildAutoIndex('x').size, 0);
});

test('isUsable keeps token-bearing rows and drops empty ones', () => {
  assert.equal(isUsable(rec({ inputTokens: 1 })), true);
  assert.equal(isUsable(rec({ cacheReadTokens: 5 })), true);       // cache-only kept
  assert.equal(isUsable(rec({ cacheWriteTokens: 5 })), true);
  assert.equal(isUsable(rec({ inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 })), false);
  assert.equal(isUsable(rec({ ts: 0, inputTokens: 1 })), false);   // ts must be > 0
});
